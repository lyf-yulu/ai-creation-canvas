...
+ [新增] 参考图片节点支持点击缩略图查看大图详情：弹窗居中预览原图并显示素材编号、格式、大小与图片尺寸（未知尺寸时按加载结果补充），支持关闭按钮、遮罩点击与 Esc 关闭。


+ [修复] 单节点修改提示词后重新生成：任务轮询遇到瞬时网络错误不再中止，会按退避间隔自动重试直到出结果；任务已永久缺失时清理保存的引用并明确报错，结果图无需切换页面回来才出现。
+ [新增] 画布节点支持拖拽右下角手柄自由调整大小（不限宽高比，最小 120×80），生成图片与视频保持画面比例居中适配节点盒（不变形、留边不裁切），素材缩略图按原比例裁切缩放，列表与控件内容自动滚动，连线与端口跟随新尺寸。
