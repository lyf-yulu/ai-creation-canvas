...
当前 Redis 边界是“跨进程执行许可”，不是持久化后台任务队列：HTTP 请求在完成 SQL 预留后提交一次提供方请求。单实例后台 worker 会租用 SQLite 中已知上游 ID 的任务并继续轮询；浏览器的任务 GET 只读 SQLite，不触发提供方轮询。尚未确认是否到达提供方的提交会保持 `submission_unknown`，不得自动重放。多副本部署仍需 Redis Streams 或等价持久队列与独立 worker，不能把现有许可计数误认为完整消息队列。
