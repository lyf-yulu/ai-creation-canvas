...
回滚顺序：先停用受影响的逻辑模型或线路，再恢复上一发布包和上一份已验证凭据池配置，最后确认 Redis 与 SQLite 健康。不得回滚或手工删除 SQLite 数据文件。当前 Redis 是跨进程提交租约，不是持久队列；单实例部署会从 SQLite 接管已经取得明确上游任务 ID 的排队或运行任务，多进程、多副本和多区域的持久队列仍是后续边界。

## 任务完成模式与恢复边界

每个新任务都会持久化一个受限的完成模式：`background` 或 `request`。具备无 Cookie 后台轮询能力的直连适配器和受管线路使用 `background`；在已经获得明确上游任务 ID 后，它们不依赖浏览器轮询，关闭页面或重启单个 Canvas 进程后仍由 worker 从 SQLite 接管。这个保证只适用于显式声明且经过验证的后台能力，不能扩大为“所有提供方都能离线恢复”。

现有 Cookie 认证的 Portal 图像、视频和人像流程使用 `request`。它们只能由任务当前 owner 发起的 `GET /api/v1/jobs/{job_id}` 用该请求携带的新鲜 Portal Cookie 推进；关闭浏览器或重启 Canvas 不会丢失已保存的任务，但在 owner 重新登录并查询之前不会继续上游轮询。Cookie 仅在当次请求内传递，不写入 SQLite、任务快照、日志或 worker，也不返回浏览器。Portal 轮询返回 `401` 或 `403` 时，Canvas 对外返回 `AUTH_REQUIRED`，释放当次租约并保留原 `queued`/`running` 状态；owner 重新登录后用新 Cookie 再次 GET 即可继续，不会重新提交。

数据库升级为增量迁移：历史未终态行的完成模式初始保持空值，应用启动时只依据服务端受信适配器注册表将可识别的行分类为 `background` 或 `request`；无法分类的行继续保持不可领取，不猜测也不重放。升级、回滚和备份仍只支持单实例 SQLite：升级前应用 SQLite 在线备份，或正常停止唯一实例后，将数据库、其引用的资产/结果和对应受限凭据配置作为同一恢复点；不得让两个实例同时挂载同一 SQLite 数据目录。

直连提交只有在适配器明确证明请求未到达提供方（`NOT_SUBMITTED`）时才能按策略重试。如果发送后连接中断、响应丢失或结果无法分类，任务进入 `submission_unknown`；相同幂等键仅返回该已存行，不再调用提供方、不换 Key 且不换线路。
...
当前 Redis 边界是“跨进程执行许可”，不是持久化后台任务队列：HTTP 请求在完成 SQL 预留后提交一次提供方请求。单实例后台 worker 仅租用 SQLite 中已知上游 ID 且完成模式为 `background` 的任务并继续轮询；普通任务 GET 只读 SQLite，但当前 owner 对 `request` 模式 Portal 任务的带 Cookie GET 会领取该任务并执行一次请求内轮询。尚未确认是否到达提供方的提交会保持 `submission_unknown`，不得自动重放。多副本部署仍需 Redis Streams 或等价持久队列与独立 worker，不能把现有许可计数误认为完整消息队列。

受管任务恢复必须重新取得任务保存的原始 Key 指纹所对应的凭据。凭据池缺失、原 Key 被移除或只剩轮换后的 Key 时，worker 会失败关闭并延迟重试，不借用其他 Key、不切换线路，也不重新提交；部署或回滚时必须把与未完成任务匹配的已验证凭据池配置一并保留，直到任务终态和提供方确认清理完成。
...
备份只在受控生产环境中处理经审计的数据目录；发布包本身可由 manifest 校验后重新构建。备份 SQLite 时必须使用 SQLite 在线备份或先正常停止唯一应用实例，并把数据库与其引用的本地资产、结果文件作为同一恢复点处理；不得只复制正在写入的数据库文件。凭据配置单独进入受限的部署密钥备份，不能混入数据目录或发布包。

部署和回滚采用单个应用实例、上一个已验证发布包及其对应的服务与凭据配置。启动新实例前先正常停止旧实例，保留原数据目录，确认文件权限和磁盘空间，再让后台 worker 接管已知上游任务；不得并行挂载同一个 SQLite 数据目录，不复制或恢复测试临时目录。`.local-real-media-data/` 只用于已忽略的本地真实媒体验证，不属于部署输入、备份或提交内容；若仓库中出现该目录或其媒体结果，应先停止发布并核查隔离。

日志只保留最少的错误码、请求 ID、时间和服务标识；禁止记录密钥、Cookie、完整提示词、上传内容、长期结果地址与原始请求体。
...
- Portal 任务轮询返回 `AUTH_REQUIRED`：任务仍保留原排队/运行状态；让 owner 重新登录后刷新任务，不要重新提交。
- 任务显示 `submission_unknown`：不要手工换 Key 或重复提交；使用相同幂等键查询原任务，等待提供方恢复查询或管理员处置。只有明确得到 `NOT_SUBMITTED` 证据时才能进入新的受控重试。
