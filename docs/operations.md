...
当前真实能力边界：Seedream 4 支持单图生成和最多 14 张有序参考图；服务端兼容提供方返回多个结果，但当前不声明一次请求多图。Seedance 2.0 支持参考图、首尾帧和符合限制的参考音频。参考视频必须先成为提供方认可的公网 URL 或 `asset://` 素材，并需要额外管理员素材上传能力，因此当前保持关闭。Ark 只允许取消仍在排队的任务，运行中的任务不支持取消。
