...
- 导入时递归拒绝任何键名为 `url`、`base_url`、`callback_url`、`api_key`、`token`、`authorization`、`headers`、`script`、`plugin` 或 `code` 的字段（大小写无关）。错误响应不回显原文件、提示词或节点参数。
...
- 导入文件、预览 payload、配置和测试夹具都不能包含真实密钥、真实用户资产、真实请求记录或生产目录引用。
...
2. 管理员导入两个给定样例后，导出的保存格式与输入的规范化 JSON/checksum 相同；节点数、连线数和节点类型库存相同。
3. 导入仓库新建的最小核心节点工作流后，导出并重新导入得到相同 checksum 和拓扑。
...
1. 导入两个给定 JSON，检查其依赖清单分别显示 MiniMaxH3/Bernini 等自定义节点为“需确认”。
