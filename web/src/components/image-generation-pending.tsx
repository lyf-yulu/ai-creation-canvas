...
export function ImageGenerationPending({ className, label, compact = false }: { className?: string; label?: string; compact?: boolean }) {
    ...
    return (
        <div className={cn("relative overflow-hidden bg-muted dark:bg-white/10", compact ? "min-h-24" : "aspect-[4/3]", className)}>
            ...
            />
            <div className="absolute left-4 top-4 flex items-center gap-2 text-[15px] font-medium text-muted-foreground dark:text-muted-foreground">
            ...
            <div className="absolute bottom-4 left-4 right-4">
                ...
                <div className="h-1.5 rounded-full bg-border/70 dark:bg-white/12">
                    <div className="h-full rounded-full bg-muted dark:bg-muted" style={{ width: `${progress}%` }} />
                </div>
            </div>
        </div>
    );
