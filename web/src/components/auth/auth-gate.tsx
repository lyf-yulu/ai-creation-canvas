...
export function AuthGate({ children }: { children: ReactNode }) {
    ...
    if (loading || (!session && !errorCode)) {
        return <main className="flex h-dvh items-center justify-center bg-[var(--c-bg)] text-sm text-[var(--c-text-3)]">正在验证登录状态…</main>;
    }
    if (!session && errorCode && !["AUTH_REQUIRED", "unauthorized"].includes(errorCode)) {
        return <main className="flex h-dvh flex-col items-center justify-center gap-3 bg-[var(--c-bg)] text-[var(--c-text)]"><p>暂时无法验证登录状态</p><button className="rounded border border-[var(--c-border-strong)] px-3 py-2 text-sm text-[var(--c-accent)]" onClick={() => useSessionStore.setState({ errorCode: null })}>重新连接</button></main>;
    }
...
export function RoleGate({ allowed, children }: { allowed: Array<"admin" | "user" | "viewer">; children: ReactNode }) {
    ...
    if (!session || !allowed.includes(session.role)) {
        return <main className="flex h-full items-center justify-center bg-[var(--c-bg)] text-[var(--c-text-3)]">页面不存在</main>;
    }
    return <>{children}</>;
}
