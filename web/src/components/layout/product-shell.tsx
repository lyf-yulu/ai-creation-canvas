import { FolderKanban, Images, LogOut, Orbit, Rows3 } from "lucide-react";
    ...
    return (
        <div className="h-dvh overflow-hidden bg-[#050806] text-[#e5f5e9]">
            <header className="flex h-14 items-center justify-between border-b border-[#193523] bg-[#08100b] px-4 md:hidden">
                ...
                <nav className="flex gap-1">{releasedNavigation.map(({ label, to }) => <NavLink key={to} to={to} className="rounded px-2 py-1 text-xs text-[#a8bbae]">{label}</NavLink>)}</nav>
            </header>
