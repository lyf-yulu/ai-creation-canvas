...
export function GenerationNodeCard({ node, onRetry }: { node: CanvasNodeData; onRetry?: (token: string) => void }) {
    ...
    return <article data-testid={result ? `result-node-${result}` : `generation-node-${node.id}`} className="overflow-hidden rounded-xl border border-[#285038] bg-[#0a140e] text-xs text-[#dceee1] shadow-xl">
        ...
        {status === "success" && node.type === "image" && node.metadata?.content ? <div className="media-surface m-3 overflow-hidden rounded-lg"><img src={node.metadata.content} alt="生成结果" className="block h-auto w-full" /></div> : status === "success" && node.type === "video" && node.metadata?.content ? <div className="media-surface m-3 overflow-hidden rounded-lg"><video aria-label="生成视频结果" className="block h-auto w-full" controls preload="metadata" src={node.metadata.content}>当前浏览器无法播放该视频。</video></div> : <div className="space-y-2 p-3"><p className="whitespace-pre-wrap text-[#b8cdbd]">{status === "error" ? node.metadata?.errorDetails : node.metadata?.prompt || node.metadata?.content}</p>{node.metadata?.requestId ? <p className="text-[#688371]">请求编号：{node.metadata.requestId}</p> : null}{status === "error" && node.metadata?.idempotencyKey && onRetry ? <button className="rounded border border-[#356b48] px-2 py-1 text-[#65e98d]" onClick={() => onRetry(node.metadata!.idempotencyKey!)}>重试</button> : null}</div>}
    </article>;
}
