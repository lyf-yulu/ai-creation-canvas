...
export function GenerationNodeCard({ node, onRetry, onDelete }: { node: CanvasNodeData; onRetry?: (token: string) => void; onDelete?: () => void }) {
    ...
    return (
        <article data-testid={result ? `result-node-${result}` : `generation-node-${node.id}`} className="flex h-full flex-col overflow-hidden rounded-xl border border-[#285038] bg-[#0a140e] text-xs text-[#dceee1] shadow-xl">
            ...
            {status === "success" && node.metadata?.content ? (
                <footer className="flex shrink-0 gap-2 border-t border-[#1c3826] p-2">
