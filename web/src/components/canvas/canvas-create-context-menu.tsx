...
export function CanvasCreateContextMenu({ menu, imageModelDisabled, videoModelDisabled, onClose, onCreate }: {
    ...
    return (
        <div ref={menuRef} role="menu" aria-label="创建节点" data-canvas-no-zoom className="fixed z-[80] min-w-52 overflow-hidden rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)] py-1 text-[var(--c-text)] shadow-2xl" style={{ left: position.left, top: position.top }} onPointerDown={(event) => event.stopPropagation()} onKeyDown={(event) => {
            ...
            <p className="px-3 py-2 text-[10px] tracking-[0.14em] text-[var(--c-text-3)]">在此处创建</p>
            {items.map((item) => (
                <button key={item.kind} role="menuitem" type="button" disabled={item.disabled} title={item.disabled ? item.reason : undefined} className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs hover:bg-[var(--c-border)] disabled:cursor-not-allowed disabled:opacity-40" onClick={() => onCreate(item.kind)} onKeyDown={(event) => {
                    if ((event.key === "Enter" || event.key === " ") && !item.disabled) { event.preventDefault(); onCreate(item.kind); }
                }}>
                    <span className="text-[var(--c-accent)]">{item.icon}</span>
