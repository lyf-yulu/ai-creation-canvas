...
export function ModelRouteEditor({ model, route, pools, onSave, onSaved, onRefresh }: Props) {
    ...
    const providers = useMemo(() => [...new Set(pools.map((pool) => pool.provider_id))].sort(), [pools]);
    const availablePools = useMemo(() => pools.filter((pool) => pool.provider_id === form.provider_id && pool.allowed_families.includes(effectiveFamily)), [effectiveFamily, form.provider_id, pools]);
    useEffect(() => {
        if (form.pool_id && !availablePools.some((pool) => pool.pool_id === form.pool_id)) setForm((current) => ({ ...current, pool_id: "" }));
    }, [availablePools, form.pool_id]);
    const submit = async () => {
    ...
    return <section className="min-w-0 rounded-xl border border-[#245a35] bg-[#07110b] p-4 text-[#e5f5e9]">
        ...
        <form className="mt-4 grid min-w-0 gap-3 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
            ...
            <div className="sm:col-span-2"><button type="submit" disabled={saving || unsupportedExisting} className="rounded bg-[#42d977] px-4 py-2 text-sm font-semibold text-[#041008] disabled:opacity-50">{saving ? "保存中…" : "保存线路"}</button></div>
        </form>
