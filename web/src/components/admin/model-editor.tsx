...
import { ADMIN_MODEL_TEMPLATES, templateForModel } from "./model-templates";
...
export function ModelEditor({ model, onSave, onSaved, onRefresh }: Props) {
    ...
    const [form, setForm] = useState({ model_id: model?.model_id || "", display_name: model?.display_name || "", introduction: model?.introduction || "", template_id: initial.id });
    ...
    return <form className="min-w-0 rounded-xl border border-[#245a35] bg-[#07110b] p-4 text-[#e5f5e9]" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        ...
        <div className="mt-4 grid min-w-0 gap-3 sm:grid-cols-2">
            ...
            <label className="min-w-0 text-sm sm:col-span-2">能力模板<select aria-label="能力模板" disabled={Boolean(model)} value={form.template_id} onChange={(event) => setForm({ ...form, template_id: event.target.value as typeof form.template_id })} className="mt-1 block w-full min-w-0 rounded border border-[#285038] bg-[#0b1710] px-3 py-2">{ADMIN_MODEL_TEMPLATES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        </div>
