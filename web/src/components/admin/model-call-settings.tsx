...
function CallingPresetCard({ model, preset, routes, pools, onCreate, onUpdate, onLifecycle, onSaved, onRefresh }: Props & { preset: AdminCallingPreset }) {
    ...
    return (
        <article className="min-w-0 rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)] p-4" aria-label={`${preset.label} 调用设置`}>
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h3 className="text-base font-semibold text-[var(--c-accent-soft)]">{preset.label}</h3>
                    <p className="mt-1 text-xs text-[var(--c-text-3)]">调用协议由内置预置维护；只可调整池、优先级、并发与启用状态。</p>
                </div>
            ...
            {duplicate ? (
                ...
                <>
                    <div className="mt-4 grid min-w-0 gap-3 sm:grid-cols-3">
                        <label className="min-w-0 text-sm">
                            ...
                            <select
                                ...
                                className="mt-1 block w-full min-w-0 rounded border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2"
                            >
                    ...
                    <button type="button" disabled={!canSave} onClick={save} className="mt-4 rounded bg-[var(--c-accent)] px-4 py-2 text-sm font-semibold text-[var(--c-accent-fg)] disabled:opacity-50">
                    ...
                    {compatiblePools.length > 0 && !hasCompatibleSelection && (
                        <p role="status" className="mt-3 text-sm text-[var(--c-text-3)]">
                            请选择兼容凭据池后保存。
                        </p>
                    )}
                </>
            ...
            {message === "conflict" && (
                <div role="alert" className="mt-3 flex flex-wrap gap-2 text-sm text-[var(--c-warning)]">
                    配置已变化，请重新加载。
                    {onRefresh && (
            ...
            <div className="mt-4 grid min-w-0 gap-2 sm:grid-cols-2" aria-label={`${preset.label} 安全池状态`}>
                {compatiblePools.map((pool) => (
                    <div key={pool.pool_id} className="min-w-0 rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] p-3 text-xs">
                        <p className="truncate text-[var(--c-text-2)]">
                            {pool.pool_id} · {pool.group}
                        </p>
...
export function ModelCallSettings(props: Props) {
    ...
    return (
        <section className="min-w-0 rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)] p-4 text-[var(--c-text)]">
            <div>
                <h2 className="text-lg font-semibold">调用设置</h2>
                <p className="mt-1 text-xs text-[var(--c-text-3)]">选择受信调用方式的兼容凭据池。真实凭据始终由部署配置管理。</p>
            </div>
