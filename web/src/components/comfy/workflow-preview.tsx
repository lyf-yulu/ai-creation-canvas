...
export function WorkflowPreview({ preview }: Props) {
    ...
    if (!preview.has_editor_layout)
        return (
            <section aria-label="工作流预览" className="rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] p-4">
                <p className="text-sm text-[var(--c-text-2)]">API 格式不包含编辑器布局，以下为稳定排序的只读摘要。</p>
                ...
                <table aria-label="工作流节点摘要" className="mt-3 w-full text-left text-sm">
                    <thead>
                        <tr className="text-[var(--c-text-3)]">
                    ...
                    <tbody>
                        {nodes.map((node) => (
                            <tr key={node.id} className="border-t border-[var(--c-panel-hover)]">
                                <td className="py-1.5 font-mono text-xs text-[var(--c-text-2)]">{node.id}</td>
                                <td className="py-1.5">{node.type}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                {preview.nodes.length > MAX_NODES && <p className="mt-2 text-xs text-[var(--c-text-3)]">为保证安全，摘要仅显示前 {MAX_NODES} 个节点。</p>}
            </section>
        );
    ...
    return (
        <section aria-label="工作流预览" className="rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] p-4">
            <p className="mb-2 text-sm text-[var(--c-text-2)]">只读通用图：仅显示节点类型与连线，不显示节点参数、JSON 或服务配置。</p>
            <svg viewBox="0 0 1800 1700" role="img" aria-label={`工作流图，${nodes.length} 个节点，${edges.length} 条连线`} className="max-h-[32rem] w-full rounded border border-[var(--c-panel-hover)] bg-[var(--c-bg)]">
                {edges.map((edge, index) => {
                    const source = positions.get(edge.source_id)!;
                    const target = positions.get(edge.target_id)!;
                    return <line key={`${edge.source_id}-${edge.target_id}-${index}`} x1={source.x + 70} y1={source.y + 28} x2={target.x + 70} y2={target.y + 28} stroke="var(--c-border-strong)" strokeWidth="2" />;
                })}
                {nodes.map((node) => {
                    const position = positions.get(node.id)!;
                    return (
                        <g key={node.id} transform={`translate(${position.x} ${position.y})`}>
                            <rect width="140" height="56" rx="6" fill="var(--c-panel)" stroke="var(--c-accent)" />
                            <text x="10" y="33" fill="var(--c-text)" fontSize="14">
                                {node.type}
                            </text>
                        </g>
                    );
