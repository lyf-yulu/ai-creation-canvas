...
export default function AdminUsagePage() {
    ...
    return <section className="mx-auto max-w-7xl px-4 py-7 sm:px-5">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{cards.map(([label, value]) => <div key={label} className="rounded-xl border border-[#1f4c2e] bg-[#09150d] p-4"><span className="block text-xs text-[#829889]">{label}</span><strong className="mt-2 block text-2xl text-[#dff6e6]">{value}</strong></div>)}</div>
        <section className="mt-6 rounded-xl border border-[#245a35] bg-[#07110b] p-4" aria-label="费用单价">
            <h2 className="text-lg font-semibold">费用单价</h2>
            <p className="mt-1 text-xs text-[#86a991]">任务成功时按此单价计费：图片按张、视频按秒，单位为分（100 分 = 1 元）。修改后仅对新完成的任务生效。</p>
            <div className="mt-4 flex flex-wrap items-end gap-4">
                <label className="text-sm text-[#b9d0c0]">
                    ...
                    <input
                        ...
                        className="mt-1 block w-40 rounded border border-[#3a7650] bg-[#0b1710] px-3 py-2 text-[#e5f5e9]"
                    />
                </label>
                <label className="text-sm text-[#b9d0c0]">
                    视频单价（分/秒）
                ...
                <button type="button" disabled={ratesStatus === "saving"} onClick={() => void saveRates()} className="rounded bg-[#42d977] px-4 py-2 text-sm font-semibold text-[#041008] disabled:opacity-40">
                    {ratesStatus === "saving" ? "正在保存…" : "保存单价"}
                </button>
                {ratesStatus === "saved" && <p role="status" className="text-sm text-[#58d881]">单价已保存。</p>}
                {ratesStatus === "failed" && <p role="alert" className="text-sm text-[#ffbd73]">单价无效或保存失败，请重试。</p>}
            </div>
        </section>
        <div className="mt-6 overflow-x-auto rounded-xl border border-[#1f3f2a] bg-[#09120c]">
            {users === null ? <p className="p-8 text-sm text-[#829889]">正在加载统计…</p> : <table className="w-full min-w-[52rem] text-left text-sm"><thead className="border-b border-[#1f3f2a] text-xs text-[#829889]"><tr><th className="p-4">账号</th><th>任务</th><th>图像</th><th>视频</th><th>成功</th><th>进行中</th><th>失败</th><th>费用(元)</th></tr></thead><tbody className="divide-y divide-[#183522]">{users.map((user) => <tr key={user.user_id}><th className="p-4 font-medium text-[#e4f5e9]">{user.display_name}<span className="ml-2 font-normal text-[#829889]">· {user.username}</span></th><td>{user.jobs}</td><td>{user.image}</td><td>{user.video}</td><td>{user.succeeded}</td><td>{user.active}</td><td>{user.failed}</td><td>{fenToYuan(user.summary?.total_cost_fen ?? 0)}</td></tr>)}</tbody></table>}
        </div>
    </section>;
}
