        ...
        <div className="mt-6 grid min-w-0 gap-5 lg:grid-cols-[17rem_minmax(0,1fr)]">
            ...
            <main className="min-w-0 space-y-5">
                ...
                {selectedModel && historicalModel && <>
                    <section className="rounded-xl border border-[#594d2a] bg-[#171408] p-4"><h2 className="text-lg font-semibold text-[#eadc91]">只读历史模型</h2><p className="mt-2 text-sm text-[#cdbf83]">{selectedModel.display_name} · {selectedModel.model_id} · 修订 {selectedModel.revision}</p><p className="mt-1 text-xs text-[#9e966d]">运行配置已清理，仅保留不可执行的审计信息。</p></section>
                    <ObjectLifecycleActions historical objectIdentity={selectedModel.model_id} objectLabel={selectedModel.display_name} enabled={false} archivedAt={selectedModel.archived_at} revision={selectedModel.revision}
                        onEnable={(revision) => changeAdminLogicalModelLifecycle(selectedModel.model_id, "enable", revision)} onDisable={(revision) => changeAdminLogicalModelLifecycle(selectedModel.model_id, "disable", revision)} onArchive={(revision) => changeAdminLogicalModelLifecycle(selectedModel.model_id, "archive", revision)} onRestore={(revision) => changeAdminLogicalModelLifecycle(selectedModel.model_id, "restore", revision)} onDelete={(revision) => deleteAdminLogicalModel(selectedModel.model_id, revision)} onPurge={(revision) => changeAdminLogicalModelLifecycle(selectedModel.model_id, "purge-runtime", revision)} onChanged={replaceModel} onDeleted={removeModel} />
                    <section className="min-w-0 rounded-xl border border-[#245a35] bg-[#07110b] p-4"><h2 className="text-lg font-semibold">历史线路</h2><div className="mt-4 grid min-w-0 gap-2 sm:grid-cols-2">{routes.map((route) => <button key={route.route_id} type="button" onClick={() => setSelectedRouteId(route.route_id)} className={`min-w-0 rounded-lg border p-3 text-left ${selectedRouteId === route.route_id ? "border-[#58ed87] bg-[#102719]" : "border-[#1e482b] bg-[#0a1710]"}`}>{route.route_id}</button>)}</div></section>
                    {selectedRoute && historicalRoute && <><section className="rounded-xl border border-[#594d2a] bg-[#171408] p-4"><h2 className="text-lg font-semibold text-[#eadc91]">只读历史线路</h2><p className="mt-2 text-sm text-[#cdbf83]">{selectedRoute.route_id} · 修订 {selectedRoute.revision}</p></section><ObjectLifecycleActions historical objectIdentity={`${selectedModel.model_id}:${selectedRoute.route_id}`} objectLabel={selectedRoute.route_id} enabled={false} archivedAt={selectedRoute.archived_at} revision={selectedRoute.revision} onEnable={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "enable", revision)} onDisable={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "disable", revision)} onArchive={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "archive", revision)} onRestore={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "restore", revision)} onDelete={(revision) => deleteAdminModelRoute(selectedModel.model_id, selectedRoute.route_id, revision)} onPurge={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "purge-runtime", revision)} onChanged={replaceRoute} onDeleted={removeRoute} /></>}
                </>}
                {selectedModel && !historicalModel && <>
                    ...
                    {selectedRoute && historicalRoute && <><section className="rounded-xl border border-[#594d2a] bg-[#171408] p-4"><h2 className="text-lg font-semibold text-[#eadc91]">只读历史线路</h2><p className="mt-2 text-sm text-[#cdbf83]">{selectedRoute.route_id} · 修订 {selectedRoute.revision}</p></section><ObjectLifecycleActions historical objectIdentity={`${selectedModel.model_id}:${selectedRoute.route_id}`} objectLabel={selectedRoute.route_id} enabled={false} archivedAt={selectedRoute.archived_at} revision={selectedRoute.revision} onEnable={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "enable", revision)} onDisable={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "disable", revision)} onArchive={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "archive", revision)} onRestore={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "restore", revision)} onDelete={(revision) => deleteAdminModelRoute(selectedModel.model_id, selectedRoute.route_id, revision)} onPurge={(revision) => changeAdminModelRouteLifecycle(selectedModel.model_id, selectedRoute.route_id, "purge-runtime", revision)} onChanged={replaceRoute} onDeleted={removeRoute} /></>}
                    {selectedRoute && !historicalRoute && <><ModelRouteEditor model={selectedModel} route={selectedRoute} pools={pools} onSave={(body) => updateAdminModelRoute(body as Parameters<typeof updateAdminModelRoute>[0])} onSaved={replaceRoute} onRefresh={() => void refreshRoute()} />
