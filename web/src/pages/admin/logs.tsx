...
export default function AdminLogsPage() {
    ...
    return (
        <section className="mx-auto max-w-6xl px-5 py-8">
            <p className="text-xs tracking-[0.2em] text-[var(--c-accent)]">ADMIN · LOGS</p>
            <h1 className="mt-2 text-2xl font-semibold sm:text-3xl">后台日志</h1>
            <p className="mt-2 text-sm text-[var(--c-text-3)]">查看服务器运行日志，支持筛选与导出。</p>
            {failed && <p role="alert" className="mt-5 rounded border border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] p-3 text-sm text-[var(--c-warning)]">日志加载失败，请重试。</p>}
            {exportFailed && <p role="alert" className="mt-3 rounded border border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] p-3 text-sm text-[var(--c-warning)]">日志导出失败，请重试。</p>}
            <div className="mt-6 flex flex-wrap items-center gap-2">
                ...
                <span className="flex items-center gap-2 text-xs text-[var(--c-text-3)]">
                    自动刷新
            ...
            <div className="mt-4 rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)]">
                <div className="flex flex-wrap items-center gap-3 border-b border-[var(--c-border)] px-4 py-3 text-xs text-[var(--c-text-3)]">
                    <span className="text-[var(--c-text-2)]">{content?.file ?? selectedFile ?? "—"}</span>
                    {content && (
                        <span>
                ...
                {files === null ? (
                    <p className="p-8 text-sm text-[var(--c-text-3)]">正在加载日志…</p>
                ) : !selectedFile ? (
                    <p className="p-8 text-sm text-[var(--c-text-3)]">暂无日志文件</p>
                ) : contentLoading ? (
                    <p className="p-8 text-sm text-[var(--c-text-3)]">正在加载日志…</p>
                ) : contentFailed ? (
                    <p className="p-8 text-sm text-[var(--c-text-3)]">日志加载失败，请重试。</p>
                ) : content && content.log_lines.length === 0 ? (
                    <p className="p-8 text-sm text-[var(--c-text-3)]">最近 {content.lines} 行内无匹配</p>
                ) : (
                    <pre className="max-h-[70vh] overflow-auto p-4 font-mono text-xs leading-5 whitespace-pre text-[var(--c-text-2)]">
