...
export default function CanvasProjectPage() {
    ...
    const invalidParam = controls.some((control) => { const value = params[control.name]; if (value === undefined) return Boolean(control.required); if (control.type === "string") return typeof value !== "string"; if (control.type === "boolean") return typeof value !== "boolean"; if ((control.type === "number" || control.type === "integer") && (typeof value !== "number" || !Number.isFinite(value) || (control.type === "integer" && !Number.isInteger(value)) || (control.minimum !== undefined && value < control.minimum) || (control.maximum !== undefined && value > control.maximum))) return true; return control.type === "enum" && !control.enum?.some((item) => Object.is(item, value)); });
    const safeParams = Object.fromEntries(controls.filter((control) => params[control.name] !== undefined).map((control) => [control.name, params[control.name]]));
