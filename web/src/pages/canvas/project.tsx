...
export default function CanvasProjectPage() {
    ...
    if (!project) {
        if (loadError)
            return (
                <main role="alert" className="flex h-full items-center justify-center bg-[var(--c-bg)] px-6 text-center text-[var(--c-warning)]">
                    {loadError.message}
                </main>
            );
        if (!projectsLoaded) return <main className="flex h-full items-center justify-center bg-[var(--c-bg)] text-[var(--c-text-3)]">正在加载画布…</main>;
        return <Navigate to="/canvas" replace />;
    }

    return (
        <div className="flex h-full min-h-0 flex-col bg-[var(--c-bg)] text-[var(--c-text)]">
            {loadError ? (
                <p role="alert" className="shrink-0 border-b border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] px-4 py-2 text-sm text-[var(--c-warning)]">
                    {loadError.message}
                </p>
            ) : null}
            {syncNotice ? (
                <p data-testid="project-sync-notice" role="status" aria-live="polite" className="shrink-0 border-b border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] px-4 py-2 text-sm text-[var(--c-warning)]">
                    {syncNotice}
                </p>
            ) : null}
            <main className="flex min-h-0 flex-1 flex-col overflow-hidden lg:grid lg:grid-cols-[152px_minmax(0,1fr)]">
                <aside data-testid="studio-palette" className="shrink-0 border-b border-[var(--c-border)] bg-[var(--c-panel)] p-2 lg:border-b-0 lg:border-r lg:p-3">
                    <div className="flex items-center justify-between gap-2 lg:block">
                        <div>
                            <Link to="/canvas" aria-label="返回项目列表" className="mb-1 inline-flex items-center gap-1 px-2 text-[11px] text-[var(--c-text-3)] hover:text-[var(--c-text)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--c-accent)]">
                                <ArrowLeft className="size-3.5" />返回项目列表
                            </Link>
                            <p className="px-2 text-xs tracking-[0.16em] text-[var(--c-accent)] lg:pt-2">NODE PALETTE</p>
                        ...
                        <div className="flex flex-wrap gap-2 lg:block lg:space-y-2">
                            <button
                                ...
                                className="flex items-center gap-2 rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-left text-xs hover:border-[var(--c-accent)] disabled:cursor-not-allowed disabled:opacity-50 lg:w-full lg:py-2.5"
                            >
                                <MessageSquareText className="size-4 text-[var(--c-accent)]" />
                                提示词节点
                            </button>
                            <button
                                disabled={readOnly}
                                ...
                                className="flex items-center gap-2 rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-left text-xs hover:border-[var(--c-accent)] disabled:cursor-not-allowed disabled:opacity-50 lg:w-full lg:py-2.5"
                            >
                                <ImagePlus className="size-4 text-[var(--c-accent)]" />
                                参考图节点
                            </button>
                            <button
                                disabled={readOnly}
                                ...
                                className="flex items-center gap-2 rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-left text-xs hover:border-[var(--c-accent)] disabled:cursor-not-allowed disabled:opacity-50 lg:w-full lg:py-2.5"
                            >
                                <Film className="size-4 text-[var(--c-accent)]" />
                                参考视频节点
                            </button>
                            <button
                                disabled={readOnly}
                                ...
                                className="flex items-center gap-2 rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-left text-xs hover:border-[var(--c-accent)] disabled:cursor-not-allowed disabled:opacity-50 lg:w-full lg:py-2.5"
                            >
                                <Music2 className="size-4 text-[var(--c-accent)]" />
                                参考音频节点
                            </button>
                            <button
                    ...
                    <p className="mt-5 hidden px-2 text-[11px] leading-5 text-[var(--c-text-3)] lg:block">集合内顺序决定 @图片N、@视频N、@音频N 的引用编号。</p>
                </aside>
                <section data-testid="studio-canvas" className="embed-surface relative min-h-0 min-w-0 flex-1">
                    ...
                    <button
                        ...
                        aria-label="打开人像资产库"
                        className="absolute right-6 bottom-20 z-30 rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-xs text-[var(--c-text-2)] hover:border-[var(--c-accent)]"
                    >
                        人像资产库
                    </button>
                    {libraryPanelOpen ? (
                    ...
                    {connectionMessage ? (
                        <p
                            ...
                            role="status"
                            aria-live="polite"
                            className="pointer-events-none absolute bottom-14 left-1/2 z-50 -translate-x-1/2 rounded-lg border border-[var(--c-border-strong)] bg-[var(--c-panel)]/95 px-3 py-2 text-xs text-[var(--c-text-2)] shadow-xl"
                        >
                            {connectionMessage}
                        </p>
                    ) : null}
                    {canvasCommandMessage ? (
                        <p data-testid="canvas-command-status" role="status" aria-live="polite" className="pointer-events-none absolute bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-lg border border-[var(--c-border-strong)] bg-[var(--c-panel)]/95 px-3 py-2 text-xs text-[var(--c-text-2)] shadow-xl">
                            {canvasCommandMessage}
                        </p>
                    ) : null}
                    {inactiveConnectionCount > 0 ? (
                        <p
                            ...
                            role="status"
                            aria-live="polite"
                            className="pointer-events-none absolute bottom-3 left-1/2 z-40 -translate-x-1/2 rounded border border-[var(--c-text-3)] bg-[var(--c-panel)]/95 px-2 py-1 text-[11px] text-[var(--c-text-2)]"
                        >
