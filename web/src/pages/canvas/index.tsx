...
export default function CanvasPage() {
    ...
    if (hydrated && !readOnly && (mode === "new" || mode === "recent")) return <main className="flex h-full items-center justify-center bg-background text-sm text-muted-foreground">正在打开画布...</main>;

    return (
        <main className="h-full overflow-auto bg-background text-foreground">
            <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-10">
                <header className="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-6 dark:border-border">
                    <div>
                ...
                {syncNotice ? <p role="status" className="rounded-lg border border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] px-4 py-3 text-sm text-[var(--c-warning)]">{syncNotice}</p> : null}
                {loadError ? <p role="alert" className="rounded-lg border border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] px-4 py-3 text-sm text-[var(--c-warning)]">{loadError.message}</p> : null}

                {!hydrated ? (
                    <section className="flex min-h-[360px] items-center justify-center border-y border-border text-sm text-muted-foreground dark:border-border">正在加载画布...</section>
                ) : projects.length ? (
                    ...
                    <section className="flex min-h-[360px] flex-col items-center justify-center border-y border-border text-center dark:border-border">
