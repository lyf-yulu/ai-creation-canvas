    ...
    return (
        <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {cards.map(([label, value]) => (
                <div key={label} className="rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)] p-4">
                    <div className="text-xs text-[var(--c-text-3)]">{label}</div>
...
function ChargedJobs({ usage }: { usage: Usage }) {
    return (
        <section className="mt-7 overflow-hidden rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)]">
            <div className="border-b border-[var(--c-panel-hover)] px-4 py-3">
                <h2 className="text-sm font-medium">已计费任务</h2>
            </div>
            {usage.jobs.length === 0 ? (
                <p className="p-6 text-sm text-[var(--c-text-3)]">暂无已计费的生成任务。</p>
            ) : (
                <ul className="divide-y divide-[var(--c-panel-hover)]">
                    {usage.jobs.map((job, index) => (
                        <li key={`${job.operation}-${job.charged_at}-${index}`} className="flex flex-wrap items-center justify-between gap-3 p-4">
                            <div>
                                <div className="text-sm text-[var(--c-text)]">{job.model_id ? `${job.model_id} · ${job.operation}` : job.operation}</div>
                                <div className="mt-1 text-xs text-[var(--c-text-3)]">
                                    {job.route_id ? `${job.route_id} · ` : ""}{job.image_count} 张图片 · {job.video_seconds} 秒视频 · {job.status}
                                </div>
                            </div>
                            <div className="text-sm font-medium text-[var(--c-accent)]">{formatFen(job.cost_fen)}</div>
                        </li>
                    ))}
                </ul>
            )}
...
function RateSettings({ rates, onSave }: { rates: UsageRates; onSave: (video: string, image: string) => Promise<boolean> }) {
    ...
    return (
        <section className="mt-7 rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)] p-5">
            <h2 className="text-sm font-medium">估算费率</h2>
            <p className="mt-1 text-xs text-[var(--c-text-3)]">价格按元输入，服务端以分保存；这是内部估算，不是供应商账单。</p>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
                <label className="text-sm text-[var(--c-text-2)]">
                    每秒视频价格（元）
                    <input aria-label="每秒视频价格（元）" inputMode="decimal" value={video} onChange={(event) => setVideo(event.target.value)} className="mt-2 block w-full rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-[var(--c-text)]" />
                </label>
                <label className="text-sm text-[var(--c-text-2)]">
                    每张图片价格（元）
                    <input aria-label="每张图片价格（元）" inputMode="decimal" value={image} onChange={(event) => setImage(event.target.value)} className="mt-2 block w-full rounded-lg border border-[var(--c-border)] bg-[var(--c-panel)] px-3 py-2 text-[var(--c-text)]" />
                </label>
            </div>
            <button type="button" disabled={saving} onClick={() => void save()} className="mt-5 rounded-lg bg-[var(--c-accent)] px-4 py-2 text-sm font-medium text-[var(--c-accent-fg)] disabled:opacity-40">
                保存价格
            </button>
        </section>
    );
...
function AdminUsageDetails({ usage }: { usage: AdminUsage }) {
    return (
        <section className="mt-7 rounded-xl border border-[var(--c-border)] bg-[var(--c-panel)] p-5">
            ...
            <p className="mt-2 text-sm text-[var(--c-text-3)]">全局汇总：{summaryText(usage.summary)}</p>
            <ul className="mt-4 divide-y divide-[var(--c-panel-hover)] border-y border-[var(--c-panel-hover)]">
            ...
            {usage.jobs.length === 0 ? (
                <p className="mt-3 text-sm text-[var(--c-text-3)]">暂无全局已计费任务。</p>
            ) : (
                <ul className="mt-3 divide-y divide-[var(--c-panel-hover)] border-y border-[var(--c-panel-hover)]">
                    {usage.jobs.map((job, index) => (
                        <li key={`${job.user_id}-${job.operation}-${job.charged_at}-${index}`} className="flex flex-wrap items-center justify-between gap-3 py-3">
                            <span>
                                <span className="block text-sm text-[var(--c-text)]">
                            ...
                            <span className="text-xs text-[var(--c-accent)]">{formatFen(job.cost_fen)}</span>
                        </li>
                    ))}
                </ul>
            )}
...
export default function UsagePage() {
    ...
    return (
        <section className="mx-auto max-w-6xl px-5 py-8">
            <p className="text-xs tracking-[0.2em] text-[var(--c-accent)]">USAGE · COST</p>
            <h1 className="mt-2 text-3xl font-semibold">生成统计</h1>
            <p className="mt-2 text-sm text-[var(--c-text-3)]">统计当前账号已完成的图片和视频任务；费用按管理员配置费率估算，不代表供应商账单。</p>
            {ownerFailed && (
                <p role="alert" className="mt-5 rounded border border-[var(--c-amber-border)] bg-[var(--c-amber-bg)] p-3 text-sm text-[var(--c-warning)]">
                    统计暂时无法完整加载，请稍后重试。
                </p>
            )}
            {adminUsageFailed && (
            ...
            {usage === null && ownerFailed ? (
                <p className="mt-7 text-sm text-[var(--c-text-3)]">统计数据暂时不可用。</p>
            ) : usage === null ? (
                <p className="mt-7 text-sm text-[var(--c-text-3)]">正在加载统计…</p>
            ) : (
                <>
