...
const safeMessage = (value: unknown, fallback: string) => {
    if (typeof value !== "string" || value.length > 160 || /[\r\n\t\u0000-\u001f]|\b[a-z][a-z0-9+.-]*:\/\/|(?:^|[\s'"(])\/(?!\/)[^\s'"()]+|[A-Za-z]:[\\/]|\b\w[\w.-]*\.(?:ts|js|py|java|go|sql):\d+\b|api[_ -]?key|authorization|bearer|secret|token|traceback|stack|exception|error:|enoent|sqlite|postgres|mysql|internal\s+(?:server|database|error)/i.test(value)) return fallback;
    return value;
};
