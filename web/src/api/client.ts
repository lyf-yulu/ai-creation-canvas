...
const safeMessage = (value: unknown, fallback: string) => {
    ...
    if (/[\\/]|[\r\n\t\u0000-\u001f]|\b(?:http|https|ftp|file|data|javascript|mailto|tel|ws|wss):|\b\w[\w.-]*\.(?:ts|js|py|java|go|sql):\d+\b|api[_ -]?key|authorization|bearer|secret|token|traceback|stack|exception|error:|enoent|sqlite|postgres|mysql|internal\s+(?:server|database|error)/i.test(value)) return fallback;
    return value;
};
