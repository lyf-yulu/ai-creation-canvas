...
_ADAPTER_TYPES = frozenset({"ark", "chiyun_openai_images"})
...
class SubmissionError(PortalUpstreamError):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            code,
            retryable=parsed not in {SubmissionDisposition.REJECTED, SubmissionDisposition.ACCEPTED},
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"code={self.code!r}, provider_task_id={self.provider_task_id!r})"
        )
...
class RejectedSubmissionError(SubmissionError, ValueError):
    ...
    def __init__(self, safe_message: str) -> None:
        ...
        super().__init__(SubmissionDisposition.REJECTED, "REQUEST_REJECTED", status_code=400)
...
class UnknownSubmissionResult(SubmissionError, InvalidUpstreamResult):
    ...
    def __init__(self) -> None:
        super().__init__(SubmissionDisposition.SUBMISSION_UNKNOWN, "INVALID_UPSTREAM_RESULT")


def local_rejection(error: ValueError) -> RejectedSubmissionError:
    ...
    return RejectedSubmissionError(safe)


def classify_submission_error(error: Exception, adapter_type: str) -> SubmissionDisposition:
...
def error_from_transport(error: httpx.HTTPError, adapter_type: str) -> SubmissionError:
    disposition = classify_submission_error(error, adapter_type)
    code = {
    ...
    return SubmissionError(disposition, code)


def error_from_response(response: httpx.Response, adapter_type: str) -> SubmissionError:
    task_id = _verified_task_id(response, adapter_type)
...
def _verified_task_id(response: httpx.Response, adapter_type: str) -> str | None:
    if adapter_type not in _ADAPTER_TYPES or len(response.content) > _MAX_ERROR_DOCUMENT:
        return None
    try:
        body = response.json()
    except ValueError:
    ...
    if not isinstance(value, str) or _PROVIDER_TASK_ID.fullmatch(value) is None:
        return None
    if adapter_type == "ark" and _ARK_TASK_ID.fullmatch(value) is None:
        return None
    return value
