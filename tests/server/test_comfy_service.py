...
async def test_adapter_uses_server_built_api_workflow_and_validates_prompt_identifier() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert json.loads(request.content) == {"prompt": API_WORKFLOW, "client_id": "request-1"}
    ...
    job = await adapter.submit(
        context(), ComfyWorkflowRequest("workflow-1", 1, API_WORKFLOW, (), "idem-1")
    )
...
def test_auth_resolver_failure_is_mapped_without_auth_reference() -> None:
    declaration = ComfyServiceDeclaration("comfy-local", "http://127.0.0.1:8188", 3, "comfy-local-auth")

    with pytest.raises(ValueError, match="ComfyUI auth header resolver failed") as raised:
        ComfyHttpWorkflowService(declaration, auth_header_resolver=lambda reference: (_ for _ in ()).throw(RuntimeError(reference)))

    assert "comfy-local-auth" not in str(raised.value)
