...
def test_only_assigned_enabled_user_can_read_preview_and_editor_export(tmp_path) -> None:
    ...
    preview = user_a.get(f"/api/v1/comfy-workflows/{workflow_id}/revisions/1/preview")
    assert preview.status_code == 200
    assert preview.json()["execution_available"] is False
    assert "widgets_values" not in preview.text
    assert user_a.get(f"/api/v1/comfy-workflows/{workflow_id}/revisions/1/export?format=editor").status_code == 200
    assert user_a.get(f"/api/v1/comfy-workflows/{workflow_id}/revisions/1/export?format=api").status_code == 400
