...
async def test_portrait_adapter_registers_both_capabilities_and_uses_cookie_per_request():
    ...
    def handler(request):
        ...
        if request.url.path.endswith("/virtual/groups"):
            return httpx.Response(201, json={"id": "group-A"})
        if request.url.path.endswith("/virtual/assets"):
    ...
    uploaded = await adapter.upload_with_cookie(context(), AssetRef("local", "portrait", "processing", "image/png"), b"png", "avatar.png", "session=a")
    ...
    assert [item[0] for item in seen] == ["/portrait/api/config", "/portrait/virtual/groups", "/portrait/virtual/assets"]
